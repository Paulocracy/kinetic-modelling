...
print("\nA) OptMDFpathway with community and single compartment")
...
for target in targets:
    ...
    mdfs[target] = optmdfpathway_result["values"]["var_B"]
    print(target, "MDF", mdfs[target])

print("\nB) CVA results in community")
optmdfpathway_base_problemB = get_base_problem()
variability_results = perform_concentration_variability_analysis(
    cobra_model=model,
    min_mdf=mdfs["community"],
    optmdfpathway_base_problem=optmdfpathway_base_problemB,
)
print(variability_results)
print("@ minimal community MDF of ", optmdfpathway_result["values"]["var_B"], "kJ/mol")

print("\nC) Check for thermodynamic bottlenecks in community")
...
for ratio_target in ("community", "single"):
    if ratio_target == "community":
        print("\nD) Community X1/X2 ratio analysis in community @ maximal community MDF")
        epsilon = 0.0
    else:
        print("\nE) Community X1/X2 ratio analysis in community @ maximal single compartment MDF + epsilon")
        epsilon = 1e-6
    ...
    print("With minimal DF of", mdfs[ratio_target] + epsilon, "kJ/mol")
